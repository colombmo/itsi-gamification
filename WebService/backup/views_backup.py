...
def output_forceGraph(request, eventID):
	if request.method == 'GET':	
		...
		# Get list and of participant to create nodes
		...
		for i,obj in enumerate(participantDict):
			...
			temp.append('{"name": "'+str(obj.tagId)+'", "group": '+str((obj.interestTag).pk)+'}')
			
		results = '{"nodes": ['+','.join(temp)+'],'
		...
		participantCount = Participant.objects.filter(event__pk=3).count()
		res = np.zeros((participantCount, participantCount), dtype = np.int)
		
		for sen in sensors:
			records = Record.objects.filter(event__pk=i_eventID).filter(sensor__pk= sen.pk).order_by('timeStamp')
			records_array = vget_timeStamp(np.array(records, dtype=np.object))
			time_array = startTime + (np.nonzero(np.bincount(vget_sec(records_array-startTime)) > 1)[0])*datetime.timedelta(seconds = 1)		#Get all absolute times when there is at least an interaction
														
			tag_couples = []			# List of all couples of tags interacting
			
			# Time-consuming loop
			t1 = time.time()
			for t in time_array:
				users_inter = np.array(np.nonzero(records_array == t)[0])	# Get all records index for this timestamp and sensor
				l = []
				for i in users_inter:
					l.append(str(records[i].tagId))
				if l.count(l[0]) != len(l):	#remove tuples of same tag
					tag_couples +=[x for x in itertools.combinations(list(set(l)),2)] # Remove duplicates with list(set(l)) and append all possible interaction combinations to lst
			print >> sys.stderr, str(int((time.time() - t1)*1000))
		...
		for i in range(participantCount):
			for j in range(i+1, participantCount):
				if res[i][j]!=0:
					temp.append('{"source":'+str(i)+', "target":'+str(j)+', "value":'+str(res[i][j])+'}')
		results += '"links":['+ ','.join(temp)+ ']}'
